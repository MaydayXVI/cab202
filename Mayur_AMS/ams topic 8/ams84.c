...
void process( void ) {
	...
	//	variable pixel_pos.
	uint8_t new_bank = (cy / 8);
	uint8_t pixel_pos = cy % 8;
	...
	int y_bit_mask = 0b00000001 << pixel_pos;
	xor_bank(new_bank, y_bit_mask);
	...
	//	bit position.
	 int x_bit_mask = 0b11111111;
	 xor_col(cx, x_bit_mask);
}
